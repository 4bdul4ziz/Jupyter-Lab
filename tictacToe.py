...
def draw(g, width=7, height=5):
    ...
    def emp(i):
        i = 2 - i
        return (
            "|".join([" " * (width - 1) + str(i * 3 + j + 1) for j in range(3)]) + "\n"
        )

    return hbar.join(
        lempty * (height // 2) + line(x) + lempty * (height // 2 - 1) + emp(i)
...
def score_board(g):
    ...
    for i in rs:
        # row win
        if g[i, 0] != 0 and all(g[i, j] == g[i, (j + 1) % s] for j in rs):
            return g[i, 0] == 1 and 100 or -100
        # col win
        if g[(0, i)] != 0 and all(g[j, i] == g[(j + 1) % s, i] for j in rs):
            return g[0, i] == 1 and 100 or -100
    # dia win
    if g[0, 0] != 0 and all(g[i, i] == g[(i + 1) % s, (i + 1) % s] for i in rs):
        return g[0, 0] == 1 and 100 or -100
    if g[0, s - 1] != 0 and all(
        g[i, s - i - 1] == g[(i + 1) % s, s - ((i + 1) % s) - 1] for i in rs
    ):
